...
# path: qwen3_moe_fused/grouped_gemm/autotuning.py @ 4c11d83
# Modified: got rid of undeclared LOOP_ORDER knob
...
def get_autotune_configs() -> list[triton.Config]:
    ...
    for m, n, k,  w, s in product(
        ...
        configs.append(
            triton.Config(
                {"BLOCK_SIZE_M": m, "BLOCK_SIZE_N": n, "BLOCK_SIZE_K": k}, num_warps=w, num_stages=s
            )
        )
    return configs
